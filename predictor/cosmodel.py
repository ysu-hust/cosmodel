...
def build_cosmodel(lambd_proxy, lambd_open, lambd_meta, lambd_data, theta_total, theta_open, theta_meta, theta_data, bp, b1, b2, npn, npp, no, device_name, obj_mode, proxy_mode, noaccept=False, timeouts=-1, diskio=None, responsedata=True):
    # with open("E:/laven/Desktop/buffer/proxyparams.log", "a+") as f:
    #     f.write("%f;%f;%f;%f;%f;%f;%f;%f;%f;%f;%f;%d;%d;%d;%s;%s;%s\n" % (lambd_proxy, lambd_open, lambd_meta, lambd_data, theta_total, theta_open, theta_meta, theta_data, bp, b1, b2, npn, npp, no, device_name, obj_mode, proxy_mode))

    def cosmodel_1obj(slo_latency):
        t = slo_latency
        ...
        def calculate_mean_srvtime(lambd):
            ...
            mean_srvtime = ((b2_d * theta_data * epsilon + b2_d * theta_data * lambd + b2_m * theta_meta * lambd + b2_o * theta_open * lambd) / lambd) + b1

            return mean_srvtime

        def laplace_srvtime_pdf(s):
            # (*\[Epsilon] is the data reading rate, \[Lambda] is the requests arriving rate*)
            # p := \[Epsilon]/\[Lambda]
            # geometricpmf[k_] := p*(1 - p)^k
            # open[t_] := \[Theta]o*o[t] + (1 - \[Theta]o)*DiracDelta[t]
            # lapopen[s_] := LaplaceTransform[open[t], t, s]
            # meta[t_] := \[Theta]m*m[t] + (1 - \[Theta]m)*DiracDelta[t]
            # lapmeta[s_] := LaplaceTransform[meta[t], t, s]
            # data[t_] := \[Theta]d*d[t] + (1 - \[Theta]d)*DiracDelta[t]
            # lapdata[s_] := LaplaceTransform[data[t], t, s]
            # lapsrvtimepdf[s_] := Sum[geometricpmf[n]*lapopen[s]*lapmeta[s]*lapdata[s]^n, {n, 0, Infinity}]
            # lapsrvtimepdf[s_] :=  Sum[geometricpmf[n]*lapopen[s]*lapmeta[s]*lapdata[s]^n, {n, 0, Infinity}]
            alpha_open = devices_alpha_open[device_name]
            alpha_meta = devices_alpha_meta[device_name]
            alpha_data = devices_alpha_data[device_name]
            ...
            laplace_srvtime = np.exp(epsilon * (-1 + laplace_part_latency_distribution(s, alpha_data, b2_d, theta_data)) / lambd) * laplace_part_latency_distribution(s, alpha_data, b2_d, theta_data) * laplace_part_latency_distribution(s, alpha_meta, b2_m, theta_meta) * laplace_part_latency_distribution(s, alpha_open, b2_o, theta_open) * laplace_b1(s)
    ...
    def cosmodel_nobj(slo_latency):
        ...
        def calculate_mean_srvtime():
            ...
            mean_srvtime = ((b2_d * theta_data * epsilon) / lambd) + b2_d * theta_data + b2_m * theta_meta + b2_o * theta_open + b1
            return mean_srvtime
        ...
        def laplace_srvtime_pdf(s):
            ...
            if usediskmodel_mg1:
                b2_o, b2_m, b2_d = calculate_mean_disk_latency_mg1()
                laplace_srvtime = np.exp(epsilon * (-1 + laplace_part_latency_distribution_mg1(s, lambd_disk, theta_data, alpha_data, b2_d)) / lambd) * laplace_part_latency_distribution_mg1(s, lambd_disk, theta_data, alpha_data, b2_d) * laplace_part_latency_distribution_mg1(s, lambd_disk, theta_meta, alpha_meta, b2_m) * laplace_part_latency_distribution_mg1(s, lambd_disk, theta_open, alpha_open, b2_o) * laplace_b1(s)
            else:
                Bbar = calculate_mean_disk_latency(lambd_disk)
                b2_o, b2_m, b2_d = Bbar, Bbar, Bbar
                laplace_srvtime = np.exp(epsilon * (-1 + laplace_part_latency_distribution(s, lambd_disk, theta_data)) / lambd) * laplace_part_latency_distribution(s, lambd_disk, theta_data) * laplace_part_latency_distribution(s, lambd_disk, theta_meta) * laplace_part_latency_distribution(s, lambd_disk, theta_open) * laplace_b1(s)
            return laplace_srvtime
